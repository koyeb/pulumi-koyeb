PROJECT_NAME := koyeb Package

SHELL            := /bin/bash
PACK             := koyeb
ORG              := koyeb
PROJECT          := github.com/${ORG}/pulumi-${PACK}
NODE_MODULE_NAME := @koyeb/${PACK}
TF_NAME          := ${PACK}
...
prepare::
	...
	if [[ "${OS}" != "Darwin" ]]; then \
		sed -i 's,github.com/pulumi/pulumi-koyeb,${REPOSITORY},g' provider/go.mod; \
...
build_sdks:: install_plugins provider build_nodejs build_python build_go
...
install_nodejs_sdk::
	yarn link --force --cwd $(WORKING_DIR)/sdk/nodejs/bin

# install_sdks:: install_dotnet_sdk install_python_sdk install_nodejs_sdk
install_sdks:: install_python_sdk install_nodejs_sdk
